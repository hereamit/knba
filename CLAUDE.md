...
- **Image optimization**: every model with an `ImageField` calls `optimize_uploaded_image()` in `save()` (EXIF-transpose, resize to ≤2200px, re-encode/compress). Uploads land in `backend/media/`.
